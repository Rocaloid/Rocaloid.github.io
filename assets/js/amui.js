/*! AmazeUI - v1.0.0 | (c) 2014 AllMobilize, Inc. | @license MIT | 2014-08-01 11:08:18 */ /*! Sea.js 2.2.1 | seajs.org/LICENSE.md */
...
define("navbar", [ "core", "zepto.extend.fx", "zepto.extend.data", "zepto.extend.selector", "qrcode", "qrcodealg", "ui.modal" ], function(require, exports, module) {
    require("core");
    var $ = window.Zepto, qrcode = require("qrcode"), modal = require("ui.modal");
    var navbarInit = function() {
        ...
        //qrcode end
        if (_parent.length) {
            $body.append($('<div id="am-navbar-moreMask"></div><ul class="am-navbar-moreList"></ul>'));
        }
...
define("qrcode", [ "./qrcodealg" ], function(require, exports, module) {
    var $ = Zepto;
    /**
     * ver 1.0.3
...
define("qrcodealg", [], function(require, exports, module) {
    // via https://github.com/aralejs/qrcode/blob/master/src/qrcodealg.js
    /**
     * 二维码算法实现
...
define("zepto.calendar", [], function(require, exports, module) {
    var $ = window.Zepto;
    var lang = {
        cn: {
            days: [ "日", "一", "二", "三", "四", "五", "六" ],
    ...
    var PickerObjects = [], InputObjects = [], ActivePickerId = -1;
    // 获取目标元素最近的父级datepicker
    // 目前只能有一个date
    ...
    var beforeMonth = function($obj) {
        ...
        var date = new Date(getPickedDate($picker));
    ...
    var getFormattedDate = function(date, date_format) {
        ...
        var d = date.getDate();
        var date_format = date_format.replace(/YYYY/gi, y).replace(/MM/g, zpadding(m)).replace(/M/g, m).replace(/DD/g, zpadding(d)).replace(/D/g, d);
        return date_format;
    };
    ...
    var draw = function($picker, option, year, month, day, hour, min) {
        ...
        for (var i = 0; i < 7; i++) {
            var $td = $("<th>");
            $td.text(daysOfWeek[/*((i + firstDayDiff) % 7)*/ i]);
            $tr.append($td);
        }
        /* Output day cells */
        ...
        for (var zz = 0; i < cellNum; i++) {
            ...
            if (isFutureOnly && isPast) {
                ...
                $td.click(function(event) {
                    ...
                    // 下一个月 td 点击无效
                    if ($(this).hasClass("day_another_month")) {
                        return;
                    }
                    $(this).addClass("active");
                    var $picker = getParentPickerObject($(this));
                    var targetDate = new Date($(this).data("dateStr"));
                    var selectedDate = new Date(getPickedDate($picker));
                    draw($picker, {
                        isAnim: false,
                        isOutputToInputObject: true
                    }, targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), selectedDate.getHours(), selectedDate.getMinutes());
                    //console.log($picker.data("dateOnly"));
                    //console.log($picker.data("isInline") == false);
                    if ($picker.data("dateOnly") == true && $picker.data("closeOnSelected")) {
                        // Close a picker
                        ActivePickerId = -1;
                        $picker.hide();
                    }
    ...
    var init = function($obj, opt) {
        ...
        if (opt.theme !== "default") {
            $picker.addClass("am-calendar-" + opt.theme);
        }
        if (!opt.current) {
        ...
        ActivePickerId = $picker.data("pickerId");
                ...
                $(input).click(function() {
                    ...
                    if (options.position === "bottom") {
                        if (_position === "relative" || _position === "absolute") {
                            $picker.parent().css("bottom", /*$input.outerHeight()*/ 10 + "px");
                        } else {
                            $picker.parent().css("bottom", 10 + "px");
                            $picker.parent().css("left", "1%");
                        }
                    } else {
                        if (_position === "relative" || _position === "absolute") {
                            $picker.parent().css("top", /*$input.outerHeight()*/ 30 + 2 + "px");
                        } else {
                            $picker.parent().css("top", $input.position().top + 30 + 2 + "px");
                            $picker.parent().css("left", "1%");
                        }
                    }
                });
    ...
    $(function() {
        $("body").click(function(event) {
            //console.log("Calendar的个数："+PickerObjects.length);
...
seajs.use(["accordion","core","divider","fastclick","figure","footer","gallery","gotop","intro","list_news","map","menu","nav","navbar","pagination","paragraph","qrcode","qrcodealg","slider","sohucs","tabs","titlebar","ui.accordion","ui.add2home","ui.alert","ui.button","ui.collapse","ui.dimmer","ui.dropdown","ui.modal","ui.offcanvas","ui.popover","ui.progress","ui.scrollspy","ui.scrollspynav","ui.smooth-scroll","ui.sticky","util.cookie","util.fullscreen","zepto.calendar","zepto.extend.data","zepto.extend.fx","zepto.extend.selector","zepto.extend.touch","zepto.flexslider","zepto.outerdemension","zepto.pinchzoom","zepto.touchgallery"]);